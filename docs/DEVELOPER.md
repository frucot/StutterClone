...
1. `project(StutterClone VERSION 0.0.1 …)` — semantic version used by CMake and JUCE (`JucePlugin_Version`).
2. `STUTTERCLONE_VERSION_STRING` (`"0.01"`) — string shown in the editor and in human-facing docs.
...
1. Bump both values together (for example `0.0.2` / `"0.02"`).
2. Add an entry to [CHANGELOG.md](../CHANGELOG.md).
3. Commit, tag (`git tag v0.02`), and push the tag.
