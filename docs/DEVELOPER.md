...
Source/
  ...
  DspChain.*            Grain → fuzz → filter → lo-fi → delay → reverb
  AdaptiveFuzzDSP.*     Two-band adaptive fuzz (after grain)
  GranularTunerDSP.*    Period-locked grain tuner (after stutter)
  ResonatorDSP.*        Comb-filter pitch engine (same slot as grain)
...
1. `project(StutterClone VERSION 0.0.9 …)` — semantic version used by CMake and JUCE (`JucePlugin_Version`).
2. `STUTTERCLONE_VERSION_STRING` (`"0.0.9"`) — string shown in the editor and in human-facing docs.
...
1. Bump both values together (for example `0.0.10` / `"0.0.10"`).
2. Add an entry to [CHANGELOG.md](../CHANGELOG.md).
3. Commit, tag (`git tag v0.0.10`), and push the tag.
...
4. `GestureDspChain` runs only while the gesture (or its fade-out) is active. Stages: `processGranular` → `processFuzz` → `processFilter` → `processLoFi` → `processDelay` → `processReverb`.
...
4. Add matching fields to `GestureDspChain::Settings` and a named `process*` stage in `DspChain.cpp` (insert it in the existing grain → fuzz → filter → lo-fi → delay → reverb order).
