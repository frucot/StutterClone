...
Source/
  ...
  DspChain.*            Fuzz → filter → lo-fi → delay → reverb
  AdaptiveFuzzDSP.*     Two-band adaptive fuzz (after stutter)
...
1. `project(StutterClone VERSION 0.0.7 …)` — semantic version used by CMake and JUCE (`JucePlugin_Version`).
2. `STUTTERCLONE_VERSION_STRING` (`"0.0.7"`) — string shown in the editor and in human-facing docs.
...
1. Bump both values together (for example `0.0.8` / `"0.0.8"`).
2. Add an entry to [CHANGELOG.md](../CHANGELOG.md).
3. Commit, tag (`git tag v0.0.8`), and push the tag.
...
4. `GestureDspChain` runs only while the gesture (or its fade-out) is active. Stages: `processFuzz` → `processFilter` → `processLoFi` → `processDelay` → `processReverb`.
...
4. Add matching fields to `GestureDspChain::Settings` and a named `process*` stage in `DspChain.cpp` (insert it in the existing fuzz → filter → lo-fi → delay → reverb order).
