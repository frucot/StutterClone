...
Source/
  ...
  DspChain.*            Fuzz → filtre → lo-fi → delay → reverb
  AdaptiveFuzzDSP.*     Fuzz adaptatif deux bandes (après le stutter)
...
4. `GestureDspChain` ne tourne que pendant la gesture (ou son fade-out). Étapes : `processFuzz` → `processFilter` → `processLoFi` → `processDelay` → `processReverb`.
...
4. Ajouter les champs correspondants à `GestureDspChain::Settings` et une étape `process*` nommée dans `DspChain.cpp` (dans l'ordre fuzz → filtre → lo-fi → delay → reverb).
